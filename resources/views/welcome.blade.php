<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-36">
            ...
            <div class="w-full md:w-3/4 lg:w-1/2">
                <div class="backdrop-blur-md bg-white/30 rounded-3xl shadow p-4">
                    <h1 class="text-slate-800 font-bold text-4xl ">Domina la tecnología web</h1>
                ...
                <div class="pt-2 relative mx-auto text-gray-600">
                    <input 
                        ...
                        placeholder="Search"
                    />
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-gray-200 font-bold py-2 px-4 rounded absolute right-0 top-0 mt-2">
                        Buscar
                    </button>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-8">
            @foreach ($courses as $item)
                <article class="bg-white shadow-lg rounded overflow-hidden">
                    <img class="h-36 w-full object-cover" src="{{ Storage::url($item->image->url) }}" alt="{{$item->title}}" />
                    <div class="px-6 py-4">
                        <h1 class="text-xl text-gray-600 mb-2 leading-6">{{ Str::limit($item->title, 40) }}</h1>
                        <p class="text-xs text-gray-500 mb-2">Prof: {{ $item->teacher->name }}</p>
                        
                        <div class="flex items-baseline">
                            <ul class="flex text-xs">
                                <li class="mr-1">
                                    <i class="fas fa-star text-{{$item->rating >= 1? 'yellow':'gray'}}-500"></i>
                                </li>
                                <li class="mr-1">
                                    <i class="fas fa-star text-{{$item->rating >= 2? 'yellow':'gray'}}-500"></i>
                                </li>
                                <li class="mr-1">
                                    <i class="fas fa-star text-{{$item->rating >= 3? 'yellow':'gray'}}-500"></i>
                                </li>
                                <li class="mr-1">
                                    <i class="fas fa-star text-{{$item->rating >= 4? 'yellow':'gray'}}-500"></i>
                                </li>
                                <li class="mr-1">
                                    <i class="fas fa-star text-{{$item->rating == 5? 'yellow':'gray'}}-500"></i>
                                </li>
                            </ul>
                        ...
                        <a href="{{route('course.show', $item)}}" class="block text-center w-full mt-6 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Más información.
                        </a>
                    </div>
                </article>
   ...
   <!-- component -->
    <footer class="bg-gradient-to-r from-gray-100 via-[#bce1ff] to-gray-100 mt-24">
